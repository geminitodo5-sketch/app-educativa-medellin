...
final cienciasViewModelProvider =
  ...
  return CienciasViewModel(ref);
});

class CienciasViewModel extends ChangeNotifier {
  final Ref _ref;

  CienciasViewModel(this._ref);
  ...
  Future<void> commandSeleccionarTema(String nombreTema,
      {double porcentajeCompletado = 100.0}) async {
    debugPrint('Tema seleccionado: $nombreTema');
    try {
  ...
  void commandVolver() => debugPrint('Ciencias: volver');
}
