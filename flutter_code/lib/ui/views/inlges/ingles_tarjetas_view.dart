...
// Módulo 3 — Tarjetas (memoria con volteo) — 3 niveles
...
final _niveles = [
  ...
  // Nivel 2 — imagen animal + imagen volumen con label
  [
    _CardData(
      id: 0,
      tipo: TipoTarjeta.imagen,
  ...
  void _tocarCarta(int idx) {
    ...
    if (_primerIdx == -1) {
      ...
      if (esPareja) {
        ...
        _bloqueado = true;
        final prev = _primerIdx;
        _primerIdx = -1;
        Future.delayed(const Duration(milliseconds: 900), () {
          if (!mounted) return;
          setState(() {
            _volteada[prev] = false;
            _volteada[idx] = false;
            _bloqueado = false;
          });
        });
...
class _FrenteTarjeta extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: encontrada ? Border.all(color: Colors.white, width: 4) : null,
        boxShadow: [
          BoxShadow(
            color: encontrada
                ? Colors.greenAccent.withOpacity(0.6)
                : Colors.black26,
            blurRadius: encontrada ? 16 : 6,
  ...
  Widget _contenido() {
    switch (data.tipo) {
      ...
      // Nivel 2: imagen de volumen + label debajo
